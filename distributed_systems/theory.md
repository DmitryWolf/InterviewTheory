...
А что система про все это знает? Она не знает про HB и не может узнать. Эта коммуникация HB происходит за пределами системы, но системе же нужно внутри себя как-то упорядочевать эти записи. Один из примеров, как делать не стоит, но пусть: скажем, что клиент 2 пишет, и в системе возникает две записи, и они возникают на разных узлах и в разное время, и система должна сама внутри себя понять, какая запись более свежая, а какая более старая. Система должна внутри себя выстроить некоторый порядок, например, снабдить эти записи версиями (или временными метками, как угодно) - **timestamps**. Есть два взгляда на порядок: у клиента есть HB, внутри системы есть глобальный порядок на версии. Мы должны согласовать два этих порядка, т.е. нужно каким-то образом сделать так, чтобы timestamps были согласованы c happens-before. Они не знают друг про друга, но у них есть общее время. Если операции упорядочены отношением HB, то значит они упорядочены во времени. Так что если мы будем соблюдать гарантию Linearizability внутри системы, то мы конечно же HB для пользователей тоже учтем. Мы про него не знаем, и мы дадим немного более сильные гарантии, чем пользователь хочет, ну и ладно, зато мы его порадуем.
...
## Concurrency. Последовательная и параллельная композиция
...
Мы доказываем, что в асинхронной модели для детерминированного алгоритма даже возможности одного сбоя достаточно, чтобы не завершаться в некоторых исполнениях. Как же нам этот факт доказать для совершенно произвольного алгоритма? Нужен какой-то способ об этом произвольном алгоритме рассуждать.
...
Если любое исполнение проходит через $S_e$, и если там вдруг все конфигурации 0-вал. (например), то это означает, что на любом пути допустимом из конфигурации $C$ алгоритм неизбежно выбирает 0, но у нас конфигурация $C$ бивалентная.
...
Лидер выбирается в начале терма и реплицирует команды, и реплики должны его слушать, и как-то понимать, что он все еще жив. Для этого каждый лидер посылает фоловерам специальные сообщения-хартбиты.
...
Второй случай: посмотрим на лидера, который записал в лог зеленого кандидата последнюю запись. $n \leq lastTerm_r < lastTerm_c < n'$ . Это означает, что лидер, который записал в зеленого кандидата последнюю запись находился в этом диапазоне, значит у него действует предположение индукции, значит у него запись из терма $n$ закоммиченная наша была, и он еще что-то сверху записал, значит она все еще есть у зеленого кандидата.
...
Почему мы вообще хотим чего-то альтернативного, когда мы построили уже 2PL, который гарантирует нам сериализуемость? Дело в том, что планировщик 2PL очень пессимистичный: он для каждого чтения и для каждой записи берет блокировку, чтобы две транзакции друг от друга изолировать. Можно делать чуть оптимистичнее.

Почему это может быть целью? Представим себе, что у нас система, в которой выполняются транзакции - это какое-то огромное распределенное хранилище. В этом хранилище есть какие-то точечные модификации, а есть какие-то огромные чтения, где мы читаем много-много данных. Т.е. пусть у нас распределенная база данных, в ней есть гигантские таблицы, и некоторые таблицы настолько большие, что мы читаем их с помощью MapReduce операции - вот настолько они большие. При этом мы хотим, чтобы пока мы читаем таблицу, мы получали ее в согласованном состоянии. Так вот, если мы залочим всю таблицу на чтение пока мы будем ее читать, то никто не сможет ничего делать больше с этой таблицей. Это неприятно. Мы бы хотели, чтобы в нашем хранилище читающие транзакции могли бы работать физически параллельно вместе с пишущими, и они бы друг друга не блокировали. Вот в 2PL у нас для этого есть конфликтующие блокировки на чтения и на записи, они друг друга исключают: читающие транзакции исключают пишущие, и наоборот (на одних и тех же ключах). 
...
## Большие данные, их хранение
Большие данные - это данные, которые характеризуются тремя свойствами:
* **Volume**: большое количество данных (недостаточно 1 сервера)
* **Variety**: неструктурированные данные (логи. csv, plaint text...)
* **Velocity**: батч-обработка, RT-обработка
![alt text](images/DaPC/lec6/1.png)

Если не хватает одного сервера, то:
* Вертикальное масштабирование: купить сервер побольше (в конце концов нужно будет масштабироваться горизонтально)
* Горизонтальное масштабирование: купить несколько серверов

Горизонтальное масштабирование:
* Увеличивается суммарная мощность
* Надо контролировать распределение задач по ресурсам
* Больше машинок => чаще что-то ломается

## GFS, HDFS
GFS (2003 г.):
* Большое количество данных (недостаточно 1 сервера)
* Нужна файловая система, работающая с несколькими серверами по сети

Затем появилась **Hadoop DFS**.

GFS - закрытая система, но есть open source реализация - HDFS.
![alt text](images/DaPC/lec6/2.png)

Три основные роли в системе:
1. Client: запускаем программы, используем файловую систему, но никаких содержательных данных мы не храним
2. DataNode: здесь хранятся данные, они разбиваются на блоки большого размера (например, 64 Mb)
3. NameNode: здесь хранятся пути к данным, храним дерево в оперативной памяти (для скорости доступа)

HDFS медленна, из-за сети и Java.

## Архитектура HDFS, Name node
Посмотрим на архитектуру HDFS.
![alt text](images/DaPC/lec6/3.png)

У нас есть Name node, она хранит дерево файловой системы, и когда мы работаем с файловой системой, мы постоянно какие-то файлы добавляем/удаляем/перемещаем. Дерево нужно часто обновлять, это обновление занимает много ресурсов процессора.

Какие проблемы архитектуры? Name node - единая точка отказа. Что с этим делать?
![alt text](images/DaPC/lec6/4.png)
1. Name node backups: чтобы у нас была не одна машина, а несколько. Чем это плохо? Нагрузка на сеть.
2. Secondary name node: это не бекап
![alt text](images/DaPC/lec6/5.png)

Есть слепок файловой системы, есть логи того, что мы с ним хотим сделать. Это обновление занимает много ресурсов процессора, поэтому Namenode отдает его на аутсорс. SNN занимается тем, что получает fsimage, хранит у себя, правда он устаревший, потому что логи изменений она получает с каким-то периодом. Т.е. SNN хранит устаревший fsimage, получает логи, обновляет fsimage, возвращает обратно в NN. Через какое-то (небольшое) время снова запрашивает логи, мерджит и возвращает на выход.

Мы добавили SNN, но с единой точкой отказа мы не разобрались еще.
![alt text](images/DaPC/lec6/6.png)

Еще попытки решить проблему:
![alt text](images/DaPC/lec6/7.png)
* HDFS Federation: у нас есть файловая система, у нас есть директории верхнего уровня, за каждую директорию отвечает своя Namenode'а
* High availability namenode: несколько namenode, одна активная, остальные синхронизируются с остальной namenode'ой, если одна падает, то алгоритмом консенсуса выбирается новый лидер

## Data nodes
Что такое Data Node? Это обычные сервера под линуксом (как и Name Node), на этих серверах есть специальные папки, в которых хранятся блоки. Так как Data Node много и блоков тоже много, при падении одной Data Node у нас получится, что часть блоков пропадет. Это разрешать нельзя, поэтому у нас есть репликации: каждый блок копируется на несколько Data Node, и если выпадет одна/несколько Data Node, мы этого даже не заметим.
![alt text](images/DaPC/lec6/8.png)
Name Node хранит информацию о расположении Data Node (это к слову о том, почему SNN не может заменить NN, потому что SNN не хранит этого). NN хранит данные о топологии сети: где какая стойка, где какой DC, и при записи блоков в HDFS она старается разнести максимально далеко блоки от друга (для отказоустойчивости, хоть и дольше из-за сети).

Хранение данных:
![alt text](images/DaPC/lec6/9.png)
У HDFS есть еще такой процесс, который запускается с некоторой периодичностью - это **Balancer**. Он проходит по всем DN и смотрит, если какая-то DN перегружена, то он блоки с нее перекидывает на другие DN (ну и сообщает об этом NN, чтобы она понимала, где лежат блоки). Помимо этого: если какая-то DN недоступна и пропала какая-то часть копий блоков, то балансер докопирует эту часть блоков.

Вот так хранятся копии:
![alt text](images/DaPC/lec6/10.png)

## Проблема маленьких файлов
На самом деле блоки не всегда одинакового размера.

* Последний блок может быть меньше
* А может быть совсем небольшим и тогда он прикрепляетсяя к предпоследнему

Известная проблема в Hadoop: проблема маленьких файлов. Если мы захотим в HDFS залить большущий файл, то HDFS сам разобьет его на блоки, сделает реплики, разнесет по нодам и с этим справиться, т.е. для сколь угодно больших данных HDFS будет работать в порядке, главное, чтобы места хватило. А что делать, если файлы маленькие? Вот слепить 2 или 3 файла в 1 блок HDFS не может, потому что мы потеряем разделитель между файлами. Поэтому что мы получаем: если файлик маленький, то на него все равно создается блок, блок меньшего размера. Но проблема не в том, что блок меньшего размера, а проблема в том, что на каждый такой блок у нас хранится метаинформация в NN, и метаинформация хранится точно такая же, что блок полноценный, что он маленький. Поэтому получается, что если у нас много маленьких блоков, то мы загрузим NN, а на диске останется много места, и это плохо.

Почему еще блоки не всегда одинакового размера? Вот у нас есть какой-то большой файл, мы его разбили на блоки. Понятно, что кратно на размер блока мы его вряд ли разделим, последний блок может быть меньше.

* Последний блок может быть меньше
* А может быть совсем небольшим и тогда он прикрепляетсяя к предпоследнему

Как у нас хранятся блоки? У каждого блока свой ID. Block Pool ID относится к топологии сети: если у нас много DC, много стоек, мы об этом сообщает в специальном конфиге в NN, и она делает несколько Block Pool'ов. Generation Stamp - это версия, т.е. мы работаем с распределенной системой, где машинки могут отваливаться, и может быть такое, что мы начали запись в HDFS, машинка отвалилась, потом она поднялась, но на ней содержится более старая версия этого блока, и система понимает, что версия более старая как раз по Generation Stamp. Дальше Size, а после размера список узлов, на которых хранятся реплики этого блока.
![alt text](images/DaPC/lec6/11.png)
У всех блоков размер одинаковый. А вот последний блок чуть-чуть меньше:
![alt text](images/DaPC/lec6/12.png)

Есть еще один случай, когда блоки другого размера: это когда последний блок совсем маленький (несколько процентов от размера блока). Ну и нам очень невыгодно хранить маленькие блоки в HDFS, поэтому он прикрепляется к предпоследнему, и предпоследний становится чуть больше.

## Чтение и запись

![alt text](images/DaPC/lec6/13.png)

Hadoop работает по парадигме Write-Once Read-Many, т.е. пишем редко, читаем часто.

Когда мы читаем, вот что происходит: мы говорим NN, что хотим прочитать такой-то файл; NN говорит, что файл находится на таких-то блоках, отсылает block_id и block_location; дальше клиент уже без участия NN идет напрямую к DN (причем к ближайшей) и выкачивает, а назад получает block_data. NN дает инструкции DN, например балансер, который в NN, может перемещать блоки. В свою очередь DN отправляют в NN хартбиты, если какой-то DN хартбиты перестает отправлять, то NN думает, что этот DN умер.

Бывает еще более вырожденный случай чтения, когда мы, например, не читаем файл какой-то, а выполняем команду по-типу **ls**. Для того, чтобы сделать ls, нам не нужно идти в DN, нам нужно пойти только в NN, и она скажет, какие файлы по названиям в этой папке хранятся.

Что касается записи? Запись происходит медленно, потому что она происходит вот так:
* Мы хотим добавить файл, идем с этим к NN
* NN нам выдает место в DN, куда мы можем записать наши блоки
* Мы начинаем писать напрямую в DN
* Записали на первую, первая отчиталась в NN; записали на вторую из первой DN, вторая отчиталась в NN; ...
* Сама команда завершится, когда на всех DN (репликах) появится блок

Такой способ записи называется **синхронная репликация**, т.е. пока мы не завершили все, мы OK не получаем. Есть еще **асинхронная репликация**, она у нас будет в Kafka - когда мы записываем информацию на какое-то подмножество серверов, получаем OK, а остальное доделываем в фоне.

## HDFS UI
![alt text](images/DaPC/lec6/14.png)

Здесь (желтое окно) выдается информация о том, что какие-то блоки могут быть потеряны: например, упало сразу несколько машинок, или произошло что-то с диском, в общем, все реплики пропали или недоступны.
![alt text](images/DaPC/lec6/15.png)

Здесь сказано, сколько мы можем хранить данных в HDFS, сколько занято, сколько используется не дня нужд HDFS: операционная система.
![alt text](images/DaPC/lec6/16.png)

Live Nodes:
![alt text](images/DaPC/lec6/17.png)
Балансер отработал и в целом на всех нодах информации поровну, кроме 5 ноды. Почему на 5 информации мало? Потому что она лежала и только недавно поднялась.

Есть [REST API](http://hadoop.apache.org/docs/r1.2.1/webhdfs.html), можно по нему обращаться к файловой системе и делать всякие операции с файлами.

## HDFS API
![alt text](images/DaPC/lec6/18.png)

* [HDFS shell](https://hadoop.apache.org/docs/current/hadoop-project-dist/hadoop-common/FileSystemShell.html)
* [HDFS Java API](https://hadoop.apache.org/docs/current/api/org/apache/hadoop/fs/FileSystem.html)
* [HDFS Web API / Web UI](https://hadoop.apache.org/docs/stable/hadoop-project-dist/hadoop-hdfs/WebHDFS.html)

Можем взаимодействовать через HDFS shell: это просто набор команд, похожих на линуксовые. HDFS Java API понятно, REST API чуть позже посмотрим

Есть всякие обертки на питоне, есть [Hadoop Pipes](https://www.ibm.com/docs/en/spectrum-symphony/7.3.1?topic=mapreduce-hadoop-pipes) на плюсах.

Разберем немного кода:
![alt text](images/DaPC/lec6/19.png)
Это REST API. Что мы делаем? Мы подключаемся к NN, идем по REST API, вводим путь к файлу и дальше вводим параметры. Нам возвращается Location, по которому нужно сделать запрос, и тогда мы получим данные.

Можно это делать все сразу, есть такой редирект:
![alt text](images/DaPC/lec6/20.png)

Еще пример команд HDFS:
![alt text](images/DaPC/lec6/21.png)

Помимо API Java, есть еще обертки:
![alt text](images/DaPC/lec6/22.png)

Есть файловая система HDFS. Как ее вообще поймать?
![alt text](images/DaPC/lec6/23.png)
Т.е. при работе на кластере у нас 2 файловые системы: локальная и HDFS. При использовании **du** у нас выдается два размера: размер самого файла (т.е. сколько данных занимает файл, если мы его выкачаем) и размер файла с учетом реплик (т.е. сколько файл реально занимает места в файловой системе).

Посмотрим на команду fsck:
![alt text](images/DaPC/lec6/24.png)

У реплик есть несколько состояний: mis-replicated (когда нет реплики), under-replicated (когда у блока меньше копий, чем мы хотели), over-replicated (больше реплик, чем мы сказали (когда нода упала, мы дореплицировали, а потом вернулась)).

## Задача 

![alt text](images/DaPC/lec6/25.png)

Оценить, потому что мы ничего не знаем про файлы.

## Статьи про HDFS
* "[Что в действительности делает SNN](http://blog.madhukaraphatak.com/secondary-namenode---what-it-really-do/)"
* [Статья про архитектуру HDFS](http://www.blogjava.net/DLevin/archive/2015/01/25/422428.html) в блоге HortonWorks (Китайская копия)
* [Статья про архитектуру HDFS](https://storageconference.us/2010/Papers/MSST/Shvachko.pdf) от Константина Швачко (Yahoo!)
* [Выдержка из курсов Яндекс про HDFS](https://nancyyanyu.github.io/posts/49a14c15/)
* T. White. "[Hadoop. The definitive guide](https://www.oreilly.com/library/view/hadoop-the-definitive/9781491901687/)", гл. 3 "Hadoop distributed filesystem". (есть русский перевод).
